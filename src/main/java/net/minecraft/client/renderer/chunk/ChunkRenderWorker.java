    ...
    protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException {
        ...
        if (lvt_2_1_ == null) {
            ...
            if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.REBUILD_CHUNK) {
                ...
                lvt_8_1_.add(this.chunkRenderDispatcher.uploadChunk(EnumWorldBlockLayer.TRANSLUCENT,
                ...
                public void onSuccess(List<Object> p_onSuccess_1_) {
                    ...
                    label21:
                    {
                        try {
                            ...
                            if (!generator.isFinished()) {
                                ChunkRenderWorker.LOGGER.warn("Chunk render task was " + generator
                                        .getStatus() + " when I expected it to be uploading; aborting task");
                            }
                        } finally {
                            generator.getLock().unlock();
                        }
