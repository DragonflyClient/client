...
public class ResourcePackRepository
{
    ...
    private final List<ResourcePackRepository.Entry> selectedPacks = Lists.newArrayList();
            ...
            for (ResourcePackRepository.Entry resourcepackrepository$entry : this.repositoryEntriesAll) {
                if (resourcepackrepository$entry.getResourcePackName().equals(s)) {
                    if (resourcepackrepository$entry.getPackFormat() == 1 || settings.field_183018_l.contains(resourcepackrepository$entry.getResourcePackName())) {
                        this.selectedPacks.add(resourcepackrepository$entry);
                        break;
                    }
    ...
    public void updateRepositoryEntriesAllAsync (Consumer<ResourcePackRepository.Entry> consumer)
    {
        List<ResourcePackRepository.Entry> found = Lists.newArrayList();
        ...
        for (File file : this.getResourcePackFiles()) {
            ...
            if (!this.repositoryEntriesAll.contains(entry)) {
                ...
                if (i > -1 && i < this.repositoryEntriesAll.size()) {
                    final Entry otherEntry = this.repositoryEntriesAll.get(i);
                    found.add(otherEntry);
                    consumer.accept(otherEntry);
                }
            }
        }

        this.repositoryEntriesAll.removeAll(found);

        for (ResourcePackRepository.Entry entry : this.repositoryEntriesAll) {
            entry.closeResourcePack();
        }

        this.repositoryEntriesAll = found;
    }
    ...
    public List<ResourcePackRepository.Entry> getSelectedPacks ()
    {
        return ImmutableList.copyOf(this.selectedPacks);
    }

    public void setRepositories (List<ResourcePackRepository.Entry> p_148527_1_)
    {
        this.selectedPacks.clear();
        this.selectedPacks.addAll(p_148527_1_);
    }
    ...
    public class Entry
    {
        ...
        {
            this.resourcePack = this.resourcePackFile.isDirectory() ? new FolderResourcePack(this.resourcePackFile) : new FileResourcePack(this.resourcePackFile);
            this.rePackMetadataSection = this.resourcePack.getPackMetadata(ResourcePackRepository.this.rprMetadataSerializer, "pack");

            try {
                this.texturePackIcon = this.resourcePack.getPackImage();
        ...
        {
            return this.resourcePack.getPackName();
        }
