...
public class GuiScreenResourcePacks extends GuiScreen
{
    ...
    public void initGui ()
    {
        ...
        final GuiScreenResourcePacks thisReference = this;
        ...
        if (!changed) {
            ...
            for (ResourcePackRepository.Entry entry : Lists.reverse(repository.getSelectedPacks())) {
                final ResourcePackListEntryFound entryFound = new ResourcePackListEntryFound(thisReference, entry);
                selectedResourcePacks.add(entryFound);
            }

            selectedResourcePacks.add(new ResourcePackListEntryDefault(thisReference));

            CompletableFuture.runAsync(() -> repository.updateRepositoryEntriesAllAsync(entry ->
            {
                if (repository.getSelectedPacks().contains(entry))
                    return;

                final ResourcePackListEntryFound entryFound = new ResourcePackListEntryFound(thisReference, entry);
                availableResourcePacks.add(entryFound);
            }));
        }
    }
