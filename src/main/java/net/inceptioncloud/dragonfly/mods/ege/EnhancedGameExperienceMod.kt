...
object EnhancedGameExperienceMod : DragonflyMod("Enhanced Game Experience") {
    ...
    @JvmStatic var fixGhostLiquids by option(true)
    ...
    override fun publishControls() = listOf(
        ...
        BooleanControl(
            !::tcpNoDelay,
            "TCP no delay",
            "Improve your network connection speed by enabling this option"
        ),
        BooleanControl(
            !::scaleResourcePackIcons,
            "Scale resource pack icons",
            "Decreases the size of resource pack icons to increase your performance and free memory"
        ),
        NumberControl(
            ::maximumPackIconSize,
            "Maximum pack icon size",
            "The maximum size that a resource pack icon is allowed to have, icons above this size are scaled down",
            min = 1.0, max = 128.0, decimalPlaces = 0, formatter = { "${it}x$it" }
        ),
        BooleanControl(
            !::disableExplicitGC,
        ...
        TitleControl(
            "Bug fixes",
            "Some issues that exist in Minecraft have been fixed by Dragonfly"
        ),
        BooleanControl(
            !::fixPlayerRotation,
            "Fix player rotation (crosshair sync)",
        ...
        BooleanControl(
            !::fixRidingHand,
            "Fix hand while riding",
            "Fixes the right hand of the player which can look weird when the player is riding"
        ),
        BooleanControl(
            !::fixGhostLiquids,
            "Fix ghost liquids",
            "Fixes an issues that caused the server to run out of sync with the client when placing lava or water on the floor"
        )
    )
}