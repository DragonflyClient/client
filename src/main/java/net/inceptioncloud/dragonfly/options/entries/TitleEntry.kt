...
class TitleEntry(val string: String) : UIListEntry() {
    ...
    override fun drawEntry(x: Int, y: Int, height: Int, width: Int) {
        Gui.drawRect(x, y, x + width, y + height, DragonflyPalette.accentNormal.rgb)
        Dragonfly.fontManager.subtitle.drawCenteredString(string, x + width / 2, y + height / 2 - 3, DragonflyPalette.background.rgb, false)
    }
