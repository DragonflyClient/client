    ...
    public void modTick(ClientTickEvent event) {
        ...
        TimeUtils.requireDelay("transition-tracking", 5000, () ->
        {
            ...
            final Map<String, Integer> groupedAmounts =
                    Dragonfly.getTransitions().stream().collect(Collectors.groupingBy(Transition::getOriginClass)).entrySet()
                            .stream() // Group Transitions by Origin
                            ...
                            .collect(Collectors
                                    .toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new)); // Collect to LinkedHashMap
