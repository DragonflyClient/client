...
public class GuiNewChat extends Gui implements Tickable
{
    ...
    public void drawChat (int updateTimes)
    {
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            final IFontRenderer fontRenderer = InceptionMod.getInstance().getFontDesign().getRenderer();
...
//                                    );
    ...
    private void setChatLine (IChatComponent component, int id, int updateCounter, boolean displayOnly)
    {
        ...
        List<IChatComponent> list = GuiUtilRenderComponents.splitText(component, i, InceptionMod.getInstance().getFontDesign().getRenderer(), false, false);
    ...
    public IChatComponent getChatComponent (int p1, int p2)
    {
        if (this.getChatOpen()) {
            ...
            if (j >= 0 && k >= 0) {
                ...
                if (j <= MathHelper.floor_float(( float ) this.getChatWidth() / this.getChatScale()) && k < InceptionMod.getInstance().getFontDesign().getRenderer().getHeight() * l + l) {
                    int i1 = k / InceptionMod.getInstance().getFontDesign().getRenderer().getHeight() + this.scrollPos;
                    ...
                    if (i1 >= 0 && i1 < this.seperateChatLines.size()) {
                        ...
                        for (IChatComponent ichatcomponent : chatline.getChatComponent()) {
                            if (ichatcomponent instanceof ChatComponentText) {
                                j1 += InceptionMod.getInstance().getFontDesign().getRenderer().getStringWidth(GuiUtilRenderComponents.removeTextColorsIfConfigured(( ( ChatComponentText ) ichatcomponent ).getChatComponentText_TextValue(), false));
