...
object GraphicsEngine
{
    ...
    fun renderDebugOverlay(content: Collection<Widget<*>>)
    {
        var index = 0
        val uppermostWidget = content.lastOrNull { it.hovered }
        content
            .filter { it.hovered && it != uppermostWidget }
            .forEach { widget ->
                ...
                RectangleRenderer.renderOutline(x, y, x + width, y + height,
                    Color(debugColors[index % (debugColors.size)]), 0.7)
                index++
            }

        uppermostWidget?.let {
            val x = (uppermostWidget as IPosition).x
            val y = (uppermostWidget as IPosition).y
            val (width, height) = Defaults.getSizeOrDimension(uppermostWidget)
            ...
            val title = uppermostWidget::class.simpleName
            val info = uppermostWidget.toInfo()
            val infoHeight = 2 + titleRenderer.height + fontRenderer.height * info.size
            val infoWidth = 2 + (titleRenderer.getStringWidth(title)).coerceAtLeast(info.map { fontRenderer.getStringWidth(it) }.max()!!)

            Gui.drawRect(x + width + 10, y - 5, x + width + 10 + infoWidth, y - 5 + infoHeight, Color(255, 255, 255, 220).rgb)
